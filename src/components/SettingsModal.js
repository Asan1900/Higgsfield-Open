...
export function SettingsModal(onClose) {
    const overlay = h('div', {
        class: 'fixed inset-0 z-[101] flex items-center justify-center bg-black/80 backdrop-blur-sm px-6 animate-fade-in'
    });

    const modal = h('div', {
        class: 'w-full max-w-md bg-[#111] border border-white/10 rounded-3xl p-8 shadow-3xl animate-fade-in-up glass'
    },
        h('div', { class: 'flex flex-col items-center text-center mb-8' },
            h('div', { 
                class: 'w-16 h-16 bg-white/5 rounded-2xl flex items-center justify-center border border-white/10 shadow-glow mb-6 animate-pulse',
            ...
            h('h2', { class: 'text-2xl font-black text-white uppercase tracking-tight mb-2' }, 'Studio Settings'),
            h('p', { class: 'text-secondary text-xs opacity-70' }, 'Configure your API preferences and workspace defaults.')
        ),
        h('div', { class: 'space-y-6' },
            h('div', { class: 'space-y-2' },
                h('label', { class: 'text-[10px] font-black text-muted uppercase tracking-widest ml-1' }, 'Muapi API Key'),
                h('div', { class: 'relative group' },
                    h('input', {
                        ...
                        'data-tooltip': 'Enter your Muapi.ai key'
                    }),
                    h('button', {
                        id: 'toggle-settings-visibility',
                        ...
                        h('svg', { id: 'eye-icon-settings', width: 20, height: 20, viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', 'stroke-width': 2 },
                            h('path', { d: 'M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z' }),
                            h('circle', { cx: 12, cy: 12, r: 3 })
                        ),
                        h('svg', { id: 'eye-off-icon-settings', width: 20, height: 20, viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', 'stroke-width': 2, class: 'hidden' },
                    ...
                    h('div', { class: 'absolute inset-x-0 bottom-0 h-px bg-gradient-to-r from-transparent via-primary/20 to-transparent opacity-0 group-focus-within:opacity-100 transition-opacity' })
                )
            ),
            h('div', { class: 'flex gap-3' },
                h('button', {
                    id: 'cancel-settings-btn',
                    class: 'flex-1 bg-white/5 text-white font-bold py-4 rounded-2xl hover:bg-white/10 transition-all border border-white/5',
                    'data-tooltip': 'Discard changes'
                }, 'Cancel'),
                h('button', {
                    id: 'save-settings-btn',
                    class: 'flex-1 bg-primary text-black font-black py-4 rounded-2xl hover:shadow-glow hover:scale-[1.02] active:scale-[0.98] transition-all',
    ...
    // Close on global overlay event
    window.addEventListener('close-all-overlays', close);
    ...
    saveBtn.onclick = () => {
        ...
        if (key) {
            ...
            saveBtn.classList.replace('bg-primary', 'bg-emerald-500');
